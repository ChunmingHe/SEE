...
class StochasticAugmentation:
    def __init__(self):
        ...
        self.scalings = [T.Resize((int(384 * 0.5), int(384 * 0.5))),  # 缩放 0.5x
                         T.Resize((384, 384)),  # 不缩放
                         T.Resize((int(384 * 2.0), int(384 * 2.0)))]  # 缩放 2x
    ...
    def apply(self, image):
        ...
        augmented_image = scaling_type(image)
...
class PseudoLabelPool:
    ...
    def compute_uncertainty_metrics(self, pseudo_label, previous_label):
        ...
        relative_uncertainty = high_uncertainty_pixels.sum() / (low_uncertainty_foreground.sum()+1e-10)
...
def train(train_loader, trainu_loader, model2, args, optimizer, epoch, writer, pseudo_label_pool, model3, previous_predictions):
    global step
    model2.train()
    model3.eval()
    loss_all = 0
    ...
    for i, ((images, gts, edges), (images2, gts2, edges2)) in enumerate(zip_longest(train_loader, trainu_loader, fillvalue=(None, None, None)), start=1):
        ...
        fused_mask, augmented_images = multi_augmentation_fusion(images2, model3, gts2, k=2)
        min_value = fused_mask.min()
        max_value = fused_mask.max()
        # fused_mask = fused_mask.cpu().detach().numpy()
        # fused_mask = np.array(fused_mask)
        # refined_masks = sam_refiner(images2, [fused_mask], sam)[0]
        # preds2_gt = torch.tensor(refined_masks)
        # 假设 augmented_images 是一个包含多个图像张量的列表
        all_preds2_gt = []
        ...
        for augmented_image in augmented_images:
            ...
            processed_images = []
            # 内层循环遍历 images2（每个元素是一个图像）
            for j in range(augmented_image.shape[0]):
                image = augmented_image[j]
                fused = fused_mask[j].squeeze(0)
                fused = fused.cpu().detach().numpy()
                fused = np.array(fused)

                use_sam_now = True
                sam = None
                sam = manage_sam_model(use_sam_now, sam)
                # 使用 sam_refiner 进行图像处理
                refined_masks, sam_masks_logits = sam_refiner(image, [fused], sam)
                ...
                # 将结果转换为 PyTorch 张量
                sam_masks_logits = torch.tensor(sam_masks_logits)
                sam = manage_sam_model(False, sam)

                # 将结果存储到列表中
                processed_images.append(sam_masks_logits)

        # 将所有处理后的图像整合成一个 batch
            preds2_gt = torch.stack(processed_images).cuda()
            preds2_gt = F.interpolate( preds2_gt, size=(384,384), mode='bilinear', align_corners=False)

            # preds2_gt = (preds2_gt - preds2_gt.min()) / (
            #             preds2_gt.max() - preds2_gt.min())
            all_preds2_gt.append(preds2_gt)
            mae = torch.mean(torch.abs(preds2_gt - gts2))
        ...
        pseudo_label_new = average_preds2_gt
...
def main(config_, save_path, args):
    ...
    for epoch in range(1, args.epoch):
        ...
        train(train_loader,trainu_loader, model_s, args, optimizer, epoch, writer,pseudo_label_pool, model_t, previous_predictions)
